...
| GPIO 21 | 터렛 모터 IN1 | DRV8833 터렛 제어 |
| GPIO 22 | 터렛 모터 IN2 | DRV8833 터렛 제어 |
...
### 버튼 조작
- **B 버튼**: 포신 당기기 (LED 깜빡임 + 효과음 2 재생, 500ms) *(A/B 버튼이 뒤바뀜)*
- **A 버튼**: 기관총 발사 (LED 점멸 + 효과음 3 재생, 1초간) *(A/B 버튼이 뒤바뀜)*
- **R1 버튼**: 헤드라이트 ON/OFF 토글

### 속도 조절
- **X 버튼 + D-PAD 위**: 좌측 트랙 속도 배율 증가 (+0.02) *(X/Y 버튼이 뒤바뀜)*
- **X 버튼 + D-PAD 아래**: 좌측 트랙 속도 배율 감소 (-0.02) *(X/Y 버튼이 뒤바뀜)*
- **Y 버튼 + D-PAD 위**: 우측 트랙 속도 배율 증가 (+0.02) *(X/Y 버튼이 뒤바뀜)*
- **Y 버튼 + D-PAD 아래**: 우측 트랙 속도 배율 감소 (-0.02) *(X/Y 버튼이 뒤바뀜)*
...
- **기관총 발사 시**: LED 500ms 간격으로 점멸 (1초간)
